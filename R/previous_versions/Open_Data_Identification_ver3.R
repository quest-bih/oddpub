...
positive_sample <- c(5, 8, 10, 19, 25, 32, 33, 37, 41, 43, 46, 48, 49, 51,
                     59, 62, 63, 67, 69, 71, 81, 83, 85, 92, 93, 110, 113, 119)
...
mean_sd <- c("as mean",
             "in mean")
mean_sd <- paste0("\\b", mean_sd)
mean_sd <- paste(mean_sd, collapse = "|") %>% tolower()
...
map_keywords <- function(publ_sentences)
{
  keywd_names <- c("available", "was_available", "not_available", "gene_db", "accession_nr", "repositories",
                   "all_data", "not_data", "source_code", "supplement", "file_formats", "mean_sd", "upon_request")
  publ_keywords <- as_tibble(publ_sentences) %>%
    ...
    add_column(mean_sd = map_lgl(publ_sentences, str_detect, pattern = mean_sd)) %>%
...
text_fragments <- function(open_data_tibble) {
  open_data_tibble <- select(open_data_tibble, c(1, 15:19))
...
open_data <- map(PDF_text_body_neg, map_keywords)
...
open_data <- open_data %>% map(mutate, com_specific_db = gene_db & accession_nr & available & !not_available & !was_available) %>%
  ...
  map(mutate, com_raw_all_data = all_data & !not_data & available & !not_available & !was_available & !upon_request & !mean_sd) %>%
  map(mutate, com_code = source_code & available & !not_available & !was_available & !upon_request) %>%
  map(mutate, com_suppl_raw_all_data_1 = supplement & ((all_data & !not_data) | source_code))

#summarze results to see if any match was found for each keyword category
open_data_detected <- map(open_data, select, c(15:19)) %>%
  map(apply, MARGIN = 2, FUN = any)

open_data_detected <- do.call(rbind, open_data_detected)

#add the last keyword combination that was
open_data_near_wd <- map_lgl(PDF_text_full_neg, str_detect, pattern = com_suppl_raw_all_data_2)
...
write_csv(open_data_sentences, paste0("../Results_ver2/open_data_sentences_ver3_neg.csv"))
